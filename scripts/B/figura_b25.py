"""
Figura B.25 — Herfindahl-Hirschman (IHH). Concentración de mercado
              del servicio de televisión restringida (2015-2023)
...
print("=== Verificación vs Anuario ===")
print(f"{'año':<6} {'CSV':>6}  {'Anuario':>8}  {'Diff':>6}")
for _, row in df.iterrows():
    a   = int(row["ANIO"])
    csv = row["IHH_TVRES_E"]
    pub = ANUARIO.get(a, None)
    diff = f"{csv - pub:+.0f}" if pub is not None else "—"
    print(f"{a:<6} {csv:>6.0f}  {pub if pub else '—':>8}  {diff:>6}")
