...
# ── 1. Cargar datos ───────────────────────────────────────────────────────────
df = pd.read_csv(
    r"C:\Users\ivan-\Documents\GitHub\anuario\datos\b.12\TD_PENETRACION_H_BAF_ITE_VA.csv", 
    encoding="latin1"
)
...
df_plot = df_plot[(df_plot["ANIO"] >= 2000) & (df_plot["ANIO"] <= 2023)].sort_values("ANIO").reset_index(drop=True)
...
ax.plot(df_plot['ANIO'], df_plot['P_BAF_E'],
...
ax.fill_between(df_plot['ANIO'], df_plot['P_BAF_E'],
                alpha=0.15, color=line_color, zorder=2)

# ── 3. Anotaciones con chip en todos los puntos ───────────────────────────────
...
# Se eliminó el bloque ax.vlines para quitar la cuadrícula vertical
...
ax.annotate("Figura B.12.", xy=(0, 1), xycoords='axes fraction',
             xytext=(15, 30), textcoords='offset points',
             fontsize=14, fontweight='bold', color='#3c3c3b', ha='left', va='center')
ax.annotate(" Accesos del Servicio Fijo de Internet por cada 100 hogares (2000-2023)",
             xy=(0, 1), xycoords='axes fraction',
             xytext=(115, 30), textcoords='offset points',
             fontsize=14, fontweight='medium', color='#3c3c3b', ha='left', va='center')
ax.set_ylabel('Accesos por cada 100 hogares', fontsize=11, color='#3c3c3b', labelpad=15, fontweight='medium')

# ── 7. Leyenda ────────────────────────────────────────────────────────────────
line_patch = plt.Line2D([0], [0], color=line_color, marker='o',
                        markersize=6, markerfacecolor=line_color,
                        markeredgecolor='none', linewidth=1.5, label='Accesos por cada 100 hogares')
...
ax.annotate('IFT con datos proporcionados por los operadores de telecomunicaciones a diciembre de cada año, del CONAPO y el INEGI.',
...
# ── 11. Guardar ───────────────────────────────────────────────────────────────
output_path = r'C:\Users\ivan-\Documents\GitHub\anuario\output\Figura_B12.png'
